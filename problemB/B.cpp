...
int main()
{
	...
	for(int overallCount=0;overallCount<testCases;overallCount++)
	{
		...
		indexPt.clear();
			...
			int wholeSchedulingScore=INT_MAX;
			...
			for(int schedulingRun=0;schedulingRun<10;schedulingRun++)
			{
				...
				//making a new scheduling and doing things again
					...
					//deciding of a scheduling
					for(int i=0;i<s;i++)
					{
						struct ScheduleNode currSch=scheduleVec[i];
						int schDepTime=currSch.startsAfter;
						string currPt=currSch.nextPt;
						int schDepOld=schDepTime;

						int startIndexPt=indexPt[currPt];
						int destIndexPt=indexPt[currSch.destination];
						...
						waitTimeSchedule=(currSch.arriveBy- currSch.startsAfter)-timeTravelSchedule;
						...
						while(schDepTime<ow && currPt!=currSch.destination)
						{
							...
							if(currSch.destination==pitstopSequenceVector[0])
							{
								...
								//DEBUG2("he3");
								int introduceRandomDelay=rand()%10;
					...
					//deciding of an appropriate matching			
			...
			cout<<currTestCaseName<<" "<<(final_out_m+final_out_d+2)<<endl;
			cout<<final_out_d<<" "<<final_out_m<<endl;
			for(int i=0;i<finalLinesOut.size();i++)
			{
				cout<<finalLinesOut[i]<<endl;
			}
			cout<<wholeSchedulingScore<<endl;
